#!/bin/sh
# Install Go with Go binary, icon, and desktop entry for Omarchy / Linux main app menu.
set -e
cd "$(dirname "$0")/.."
...
if [ -f assets/AppIcon.png ]; then
    cp assets/AppIcon.png ~/.local/share/icons/hicolor/512x512/apps/go-with-go.png
fi
...
echo "Added icon and desktop shortcut to ~/.local/share/applications/go-with-go.desktop"
